...
⚠️ **重要提示**：生产环境部署前必须完成以下安全加固措施：

- **修改默认管理员密码**：首次部署后立即修改默认密码（默认用户名/密码: admin/admin）
- **密码加密存储**：当前密码为明文存储，强烈建议使用 BCrypt、PBKDF2 或 Argon2 等加密算法
- **SQL 注入防护**：项目已使用 MyBatis 参数化查询，但需要审查所有 SQL 语句
- **XSS 防护**：对用户输入进行过滤和转义，特别是文章内容和评论
- **CSRF 防护**：添加 CSRF Token 验证机制
- **HTTPS 部署**：生产环境必须使用 HTTPS 加密传输
- **文件上传安全**：验证文件类型、大小和内容，防止恶意文件上传
- **Session 安全**：配置合理的 Session 超时时间和安全标志
